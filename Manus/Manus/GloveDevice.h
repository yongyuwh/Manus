...
class GloveDevice : public IDevice
{
private:
	bool m_running;
...
public:
	GloveDevice(const char* device_path);
	virtual ~GloveDevice();

	virtual void Connect() override;
	virtual void Disconnect() override;
	virtual bool IsRunning() const override { return m_running; }
	virtual const char* GetDevicePath() const override { return m_device_path; }
	virtual bool GetData(GLOVE_DATA* data, device_type_t device, unsigned int timeout) override;
	virtual bool GetFlags(uint8_t &flags, device_type_t device, unsigned int timeout) override;
	virtual bool GetRssi(int32_t &rssi, device_type_t device, unsigned int timeout) override;
	virtual bool GetBatteryVoltage(uint16_t &voltage, device_type_t device, unsigned int timeout) override;
	virtual bool GetBatteryPercentage(uint8_t &percentage, device_type_t device, unsigned int timeout) override;

	virtual bool IsConnected(device_type_t device) override;
	
	virtual bool SetVibration(float power, device_type_t dev, unsigned int timeout) override;
	virtual bool SetFlags(uint8_t flags, device_type_t device) override;
	virtual bool PowerOff(device_type_t device) override;

private:
	static void DeviceThread(GloveDevice* dev);
	void UpdateState();
};
